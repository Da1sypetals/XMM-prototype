...
class SumOperator:
    ...
    def compile(
        self, build_dir: Optional[str] = None, identifier: Optional[str] = None
    ):
        if identifier is None:
            identifier = str(uuid.uuid4()).replace("-", "")
        assert valid_identifier(identifier), (
            f"Invalid Identifier: {identifier}, pattern required: ^[A-Za-z0-9_]+$"
        )
        ...
        self.module = torch.utils.cpp_extension.load_inline(
            name=f"xmm_operator_{identifier}",
