...
def generate_expr(nrow, ncol, expression):
    ...
    assert ss == identifiers_set, (
        f"Incorrect variable use!\n\n[nrow = {nrow}, ncol = {ncol}]"
    ...
    derivative_ast_trees = {
        id: sympy2ast(expr) for id, expr in derivative_sympy_exprs.items()
    }

    # Step 5: Convert all AST expressions (original and derivatives) into Taichi expressions
    CUDA_expr = ast2CUDAexpr(ast_tree.body)
    CUDA_derivatives = {
        id: ast2CUDAexpr(tree) for id, tree in derivative_ast_trees.items()
    }
